...
export function ExecutiveReportDocument({
  ...
  const { kpis, sectorDistribution, offerDistribution, seekDistribution, meetingLedger } =
    snapshot
  ...
  return (
    <Document>
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.kpiRow}>
          <View style={styles.kpiBox}>
            <Text style={styles.kpiLabel}>Nivel promedio de satisfacción (1–5)</Text>
            <Text style={styles.kpiValue}>{kpis.satisfactionScore} ⭐</Text>
          </View>
          <View style={styles.kpiBox}>
        ...
        <Text style={{ fontSize: 9, marginTop: 12, lineHeight: 1.5 }}>
          El índice de intenciones de vinculación ({kpis.allianceIndex}%) refleja evaluaciones con
          expectativa de alianza alta o media. La tasa de asistencia efectiva ({kpis.attendanceRate}
          %) se calcula sobre reuniones confirmadas ya finalizadas. Este informe consolida datos en
          tiempo real hasta el momento de generación.
        </Text>
